...
var CucumberExpressionTokenizer_1 = __importDefault(require("./CucumberExpressionTokenizer"));
    ...
    switch (token.type) {
        ...
        case Ast_1.TokenType.endOptional:
            return {
                consumed: 1,
                ast: [
                    new Ast_1.Node(Ast_1.NodeType.text, undefined, token.text, token.start, token.end),
                ],
            };
 ...
 * parameter := '{' + text* + '}'
 */
var parseParameter = parseBetween(Ast_1.NodeType.parameter, Ast_1.TokenType.beginParameter, Ast_1.TokenType.endParameter, [parseName]);
/*
 * optional := '(' + option* + ')'
 * option := optional | parameter | text
 */
var optionalSubParsers = [];
var parseOptional = parseBetween(Ast_1.NodeType.optional, Ast_1.TokenType.beginOptional, Ast_1.TokenType.endOptional, optionalSubParsers);
...
function parseAlternativeSeparator(expression, tokens, current) {
    ...
    return {
        consumed: 1,
        ast: [
            new Ast_1.Node(Ast_1.NodeType.alternative, undefined, token.text, token.start, token.end),
        ],
    };
}
...
var parseAlternation = function (expression, tokens, current) {
    ...
    var result = parseTokensUntil(expression, alternativeParsers, tokens, current, [Ast_1.TokenType.whiteSpace, Ast_1.TokenType.endOfLine, Ast_1.TokenType.beginParameter]);
    var subCurrent = current + result.consumed;
    if (!result.ast.some(function (astNode) { return astNode.type == Ast_1.NodeType.alternative; })) {
        return { consumed: 0 };
    }
 ...
 */
var parseCucumberExpression = parseBetween(Ast_1.NodeType.expression, Ast_1.TokenType.startOfLine, Ast_1.TokenType.endOfLine, [parseAlternation, parseOptional, parseParameter, parseText]);
var CucumberExpressionParser = /** @class */ (function () {
    ...
    CucumberExpressionParser.prototype.parse = function (expression) {
        var tokenizer = new CucumberExpressionTokenizer_1.default();
...
function parseBetween(type, beginToken, endToken, parsers) {
    return function (expression, tokens, current) {
        ...
        var ast = [new Ast_1.Node(type, result.ast, undefined, start, end)];
...
function parseTokensUntil(expression, parsers, tokens, startAt, endTokens) {
    ...
    while (current < size) {
        ...
        ast.push.apply(ast, __spread(result.ast));
    }
